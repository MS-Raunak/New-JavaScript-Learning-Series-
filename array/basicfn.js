let arr = [10, 2, 30, 4, 50];
console.log("Given Array:", arr);
//length attribute to find length
//console.log(arr.length); 

//Basic Inbuilt Functions##############################

//push():- used to insert one or multiple element at the ending index of given array
arr.push(60,70);
console.log("Array after adding some elements at the last: ", arr);

//pop():- used to remove last single element of an array
arr.pop();
console.log("Array after removing last ele from last", arr);

//shift():- used to remove first single element of an array
arr.shift();
console.log("Array after removing first ele from last", arr);

//unshift(): used to insert one or multiple element at the beginning index of given array
arr.unshift(0,1)
console.log("Array after adding some elements at the last: ", arr);
